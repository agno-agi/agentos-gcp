...
5. **AgentOS UI.** Chat with agents, inspect sessions, traces, memory, and evals from the control plane at [os.agno.com](https://os.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-gcp&utm_content=agentos-gcp&utm_term=gcp).
...
6. Ask me which frontends I want connected, then set up the ones I pick:
   ...
   - Claude and ChatGPT apps (web or desktop): their sessions run in the cloud and can't reach localhost, so work with me on deploying to production first using ./scripts/gcp/up.sh (needs the gcloud CLI authenticated with a project selected and billing enabled, plus Docker running; the script pauses while I mint a JWT key at os.agno.com, and generates MCP_CONNECT_SECRET — the OAuth consent secret — into .env.production). Then I add https://<cloud-run-url>/mcp as a custom connector in the chat app's connector settings and approve the consent page with that secret.
...
1. Open [os.agno.com](https://os.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-gcp&utm_content=agentos-gcp&utm_term=gcp) and sign in.
...
You can run the platform anywhere that supports containerized images. For the lightest lift, the codebase comes with scripts to deploy the platform to [Google Cloud Run](https://cloud.google.com/run).
...
> **Cost note.** Always-on Cloud Run at this template's sizing (2 vCPU / 4 GiB, min 1 instance, no CPU throttling) is ≈$110/mo list price. The budget knob is 1 vCPU / 2 GiB at ≈$58/mo — edit the `--cpu`/`--memory` flags in [`scripts/gcp/up.sh`](scripts/gcp/up.sh). Cloud SQL `db-g1-small` adds ≈$25-35/mo.
...
This enables the required APIs, creates an Artifact Registry repo and pushes a locally-built image to it, provisions Cloud SQL Postgres on a private IP, stores your keys in Secret Manager, and deploys the Cloud Run service. The first run takes ~15-20 minutes: the one-time VPC peering is ~5 minutes and the Cloud SQL instance another 5-10. The script pauses and asks for a JWT verification key for authentication (see next section).
...
If you run non-interactively or skip the prompt, you can sync environment variables later with `./scripts/gcp/env-sync.sh`.
...
For **claude.ai and ChatGPT (web)**, the platform serves its own OAuth authorization server on `/mcp` — `up.sh` generated the login secret for it (`MCP_CONNECT_SECRET` in `.env.production`) during deploy. Add `https://<your-cloud-run-url>/mcp` as a custom connector in the chat app's connector settings. **Leave the form's optional OAuth fields (client ID / client secret) empty** — the platform registers the app automatically. The connect secret is typed exactly once, on the consent page that opens after you click **Connect**.

### 5. Verify

You can check the logs in the Cloud Console, or by running the following command:
...
It rebuilds the image, pushes it, and rolls the service to the new build. Env vars, secrets, sizing, and networking carry over from the running revision.
...
It pushes every variable from `.env.production` (or an env file you pass as an argument) in one update: secret-shaped keys go to Secret Manager, the rest become plain env vars.
...
Set `authorization=False` in [`app/main.py`](app/main.py) and redeploy. Use this only inside a private VPC behind another auth layer. Without it, anyone who guesses your Cloud Run URL can access your platform.
...
| `AGENTOS_URL` | no | `http://127.0.0.1:8000` | Scheduler base URL. `scripts/gcp/up.sh` sets it to the Cloud Run URL right after the first deploy and writes it back into your env file; set by hand only for a custom domain or tunnel. Also the public origin OAuth metadata derives from when `MCP_CONNECT_SECRET` is set. |
...
- [Agno documentation](https://docs.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-gcp&utm_content=agentos-gcp&utm_term=gcp)
- [AgentOS introduction](https://docs.agno.com/agent-os/introduction?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-gcp&utm_content=agentos-gcp&utm_term=gcp)
