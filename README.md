...
For **claude.ai and ChatGPT (web)**, follow the remote MCP server registration process for `https://<your-cloud-run-url>/mcp` and authenticate using OAuth.
...
| `AGENTOS_URL` | no | `http://127.0.0.1:8000` | Scheduler base URL. `scripts/gcp/up.sh` sets it to the Cloud Run URL right after the first deploy and writes it back into your env file; set by hand only for a custom domain or tunnel. |
